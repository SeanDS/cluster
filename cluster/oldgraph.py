...
class Graph (Notifier):
    ...
    def _mincutphase(self, graph):
            ...
            while len(todo) > 0:
                # pick vertex
                todolist = list(todo)
                todolist = [x for x in todolist if todo[x] > 0]   #connected!
...
def random_graph(vertices, edges, bidirectional = False, basename="v"):
    """generate a random graph with given number of
    vertices and edges"""
    g = Graph()
    for i in range(0, vertices):
        g.add_vertex(basename+str(i+1))
    for i in range(0, edges):
        a = random.randint(1, vertices)
        b = random.randint(1, vertices)
        if bidirectional:
            g.add_bi(basename+str(a), basename+str(b))
        else:
            g.add_edge(basename+str(a), basename+str(b))
    return g

def complete_graph(nvertices, monocycles=False, basename="v"):
    g = Graph()
    for i in range(nvertices):
        for j in range(nvertices):
            if i!=j and monocycles==False:
                g.add_bi(basename+str(i),basename+str(j))
    return g

def _test():
    print("test graph module")
    ...
    print("random graph v1-v20 with 50 edges")
    g = FanGraph(random_graph(20,50))
    print("path v1-v20:", g.path("v1","v20"))
    print("path v1-v1:", g.path("v1","v1"))
    print("path v1-v1 in reverse graph:",g.reverse().path("v1","v1"))
    print("removing v1")
    g.rem_vertex('v1')
    print("print graph")
    print(g)
    for i in g.fanin_numbers():
            print("vertices with fan-in",i)
            print(g.infan(i))
    for i in g.fanout_numbers():
            print("vertices with fan-out",i)
            print(g.outfan(i))

    print("generating a 10000 node 60000 edge graph")
    g = FanGraph(random_graph(10000,60000))
    print("roots:")
    print(g.roots())
    print("leafs:")
    print(g.leafs())
